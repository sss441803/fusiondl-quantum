...
(y_prime_validate, y_gold_validate, disruptive_validate, roc_validate,
 loss_validate) = mpi_make_predictions_and_evaluate(conf, shot_list_validate,
                                                 loader, custom_path)
...
g.print_unique('Validation Loss: {:.3e}'.format(loss_validate))
g.print_unique('Validation ROC: {:.4f}'.format(roc_validate))
g.print_unique('Test Loss: {:.3e}'.format(loss_test))
g.print_unique('Test ROC: {:.4f}'.format(roc_test))

if g.task_index == 0:
    disruptive_validate = np.array(disruptive_validate)

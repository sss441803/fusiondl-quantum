...
class PerformanceAnalyzer():
    ...
    def calculate_sig_roc(self,yprime = None, ygold=None,mode =None,pred_index =1,truth_positive_threshold = 8):
       # early_th_tr,correct_th_tr,late_th_tr,nd_th_tr = self.get_threshold_arrays(all_preds_tr,all_truths_tr,tearing_tr)
        preds_te = [p[:,0] for p in yprime]
        truth_te = [p[:,0] for p in ygold]
        tearing_te = [np.amax(p[:,0])>truth_positive_threshold for p in ygold]
        ...
        return thresh_range, TPR, FPR,auroc
    def check_modes(self,mode = 'test',bar_plot = False):
...
#'Locked mode amplitude':
                       ...
                       lm_plot=lm_plot[:,0]
                plot_des=['plasma current','internal inductance','stored energy','Locked mode amplitude']
...
thresh_range,TP,FP,roc_are = analyzer.calculate_sig_roc(yprime = yprime,ygold=ygold)

...
from .monolithic_QConv_Kernel import ConvKernel, entangle_mat
...
class testQ_MulIn1Out_Conv1D(nn.Module):
...
class testQConv1D(nn.Module):
    def __init__(self, in_channels, out_channels, kernel_size, stride=1):
        ...
        for _ in range(self.out_channels):
            self.convs.append(testQ_MulIn1Out_Conv1D(self.entangle_matrix, in_channels, kernel_size, stride))
        self.convs = nn.ModuleList(self.convs)
    def forward(self, x):
        device = x.device
        output = torch.tensor([]).to(device)

...
def rot_mat(weights: nn.Parameter) -> torch.Tensor:
    ...
    mats = torch.cat([ry_mat(weights[qubit]).unsqueeze(0) for qubit in range(n_qubits)], 0)
...
class ConvKernel(nn.Module):
    def __init__(self, entangle_matrix, n_qubits):
        ...
        self.weight = nn.Parameter(torch.randn(3+n_qubits, dtype=torch.float32))
    ...
    def forward(self, inputs):
        ...
        if device == 'cpu':
            matrix = torch.matmul(self.entangle_matrix, rot_matrix)
        else:
            matrix = torch.matmul(self.entangle_matrix[int(device[-1])], rot_matrix)
